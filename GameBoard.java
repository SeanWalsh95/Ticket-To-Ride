...
public class GameBoard {
	...
	public GameBoard(ArrayList<Player> playersIn) {
		...
		try (Scanner sc = new Scanner(new File("resources\\Routes.txt"))) {
			...
			System.err.println(
					"(ERR Tech): Cannot find file: resources\\Routes.txt");
		}
		...
		try (Scanner sc = new Scanner(new File("resources\\Cities.txt"))) {
			...
			System.err.println(
					"(ERR cities): Cannot find file: resources\\Cities.txt");
		}

		techAvail = new ArrayList<Card>();
		try (Scanner sc = new Scanner(
				new File("resources\\Technologies.txt"))) {
			while (sc.hasNext())
		...
		players = playersIn;
		/*
		 * players = new ArrayList<Player>(); players.add(new
		 * Player(0,"bob",Color.BLUE)); players.add(new
		 * Player(1,"joe",Color.GREEN)); players.add(new
		 * Player(2,"sam",Color.RED));
		 */
	}
	 ...
	 *
	 * @param cityA
	 *            One of the cities that connects the desired route
	 * @param cityB
	 *            One of the cities that connects the desired route
	...
	public boolean buyRoute(City cityA, City cityB) {
		...
		if (success == true) {
			if (!hasTech(curPlayer, Technology.Thermocompressor))
				endTurn();
			else
				discardTech(curPlayer, Technology.Thermocompressor);
		}
		return success;
	}
	 ...
	 *
	 * @param route
	 *            The route you want to buy
	 * @param player
	 *            The player doing the buying
	 * @param locoCost
	 *            The number of locomotives needed to buy the route
	...
	private boolean purchase(Route route, Player player, int locoCost,
		...
		if (hasTech(player, Technology.DieselPower))
			trainReq--;
		RouteColor colorOfClaim = route.color;
		if (trainReq > trains.size())
			return false;
		...
		for (int i = 0; i < trains.size(); i++) {
			if (((Train) trains.get(i)).color == RouteColor.NEUTRAL)
				locos++;
		}
		if (locos < locoCost)
			return false;
		locos -= locoCost;
		if (trainReq > (locos + properColor))
			return false;
		for (Card train : trains) {
	 ...
	 *
	 * @param trains
	 *            The ArrayList you want to search
	...
	private RouteColor getMostColor(ArrayList<Card> trains) {
		...
		int orange = 0;
		for (Card train : trains) {
			if (((Train) train).color == RouteColor.YELLOW)
				yellow++;
			else if (((Train) train).color == RouteColor.PINK)
				pink++;
			else if (((Train) train).color == RouteColor.RED)
				red++;
			else if (((Train) train).color == RouteColor.BLACK)
				black++;
			else if (((Train) train).color == RouteColor.BLUE)
				blue++;
			else if (((Train) train).color == RouteColor.GREEN)
				green++;
			else if (((Train) train).color == RouteColor.WHITE)
				white++;
			else if (((Train) train).color == RouteColor.ORANGE)
				orange++;
		}
		if (yellow >= pink && yellow >= red && yellow >= black && yellow >= blue
				&& yellow >= green && yellow >= white && yellow >= orange) {
			return RouteColor.YELLOW;
		} else if (pink >= yellow && pink >= red && pink >= black
				&& pink >= blue && pink >= green && pink >= white
				&& pink >= orange) {
			return RouteColor.PINK;
		} else if (red >= pink && red >= yellow && red >= black && red >= blue
				&& red >= green && red >= white && red >= orange) {
			return RouteColor.RED;
		} else if (black >= pink && black >= red && black >= yellow
				&& black >= blue && black >= green && black >= white
				&& black >= orange) {
			return RouteColor.BLACK;
		} else if (blue >= pink && blue >= red && blue >= yellow
				&& blue >= black && blue >= green && blue >= white
				&& blue >= orange) {
			return RouteColor.BLUE;
		} else if (green >= pink && green >= red && green >= yellow
				&& green >= blue && green >= black && green >= white
				&& green >= orange) {
	 ...
	 *
	 * @param player
	 *            The player whose tech you want to check
	 * @param route
	 *            The route whose requirements you want to check
	 * @param cityA
	 *            One of the cities connected to the route
	 * @param cityB
	 *            One of the cities connected to the route
	 * @return Method returns true if the player can buy the route, false if not
	 */
	private boolean techChecker(Player player, Route route, City cityA,
			City cityB) {
		...
		if (cityA.region != Region.England) {
			if (cityA.region == Region.Scotland) {
				...
				if (!hasTech(player, Technology.WalesConcession))
					return false;
			} else {
		...
		if (cityB.region != Region.England && cityB.region != cityA.region) {
		...
		if (route.trainRequirement == 3) {
			if (!hasTech(player, Technology.MechanicalStoker))
				return false;
		...
		if (route instanceof FerryRoute) {
			if (!hasTech(player, Technology.Propellers))
				return false;
		}
	 ...
	 *
	 * @param player
	 *            The player you want to check if they own the tech
	 * @param techName
	 *            The name of the tech you want to find
	 ...
	 *
	 * @param player
	 *            The player you want to remove the tech from
	 * @param techName
	 *            The name of the tech you want to discard
	 */
	 ...
	 *
	 * @param cityA
	 *            The name of one of the cities you want to connect
	 * @param cityB
	 *            The name of one of the cities you want to connect
	...
	public Route getRoute(CityName cityA, CityName cityB) {
		for (int i = 0; i < routes.size(); i++) {
			if ((routes.get(i).cityA.equals(cityA)
					&& routes.get(i).cityB.equals(cityB))
					|| (routes.get(i).cityB.equals(cityA)
							&& routes.get(i).cityA.equals(cityB)))
				return routes.get(i);
		}
		return null;
	}
	 ...
	 *
	 * @param tech
	 *            The tech that the player selects to buy
		...
		if (trainsToSpend.size() < locoCost)
			...
			if (trainsSpent.size() == locoCost) {
				...
				if (hasTech(curPlayer, Technology.Booster)) {
					if (locoCost == trainsSpent.size()
							+ (trainsToSpend.size() / 3)) {
	 ...
	 *
	 * @param player
	 *            The player whose destination cards you want to check
	 ...
	 *
	 * @param playerID
	 *            The int value associated with the player and routes to
	 *            indicate ownership
	 * @param start
	 *            The starting city that you want to build a path from
	 * @param dest
	 *            The destination city that you want to reach with your path
	 * @param visited
	 *            An ArrayList of all the cities the algorithm has visited while
	 *            trying to get from start to dest
	 ...
	 *
	 * @param player
	 *            The player whose route you want to check.
	 ...
	 * @param route
	 *            The route you want to add to the long route
	 * @param visited
	 *            The list of routes already included in the current route
	...
	/**
	 * The selectTrains method is used to allow the user to select trains they
	 * want to spend
	 *
	 * @return The ArrayList containing the train cards that the player wants to
	 *         spend
	 */
		...
		for (int i = 0; i < p.heldTrainCards.size(); i++) {
			if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(null,
					"Do you want " + "to" + " " + "add a "
							+ ((Train) p.heldTrainCards.get(i)).color + " "
							+ "train to your pool?",
					"choose one", JOptionPane.YES_NO_OPTION)) {
	...
	/**
	 * This method is for retrieving the information for the current player to
	 * be used to track player turns in GUI
	 * 
	 ...
	 * 
	 * @param cn
	 *            The name of the city you want to find
	 * 
	...
	/**
	 * Method used to retrieve the Tech object associated with the given
	 * Technology
	 * 
	 * @param t
	 *            The name of the Tech you want to find
